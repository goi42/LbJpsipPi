...
void makeplots2_wonky_fordisplay(TString runmode ="d", TString drawopt=""){
  ...
  for(int i=0; i<nLayers; i++){
    ...
    if(L[i].name=="decaymode") {
      for(int j=0;j<nFiles;j++) {
        L[i].add_element(&f[j].quality["filetype"]);
      }
  ...
  for(int ci=0; ci<nCanvases; ci++){
    ...
    for(int hi=0; hi<nhpc; hi++){
      ...
      cout<<"creating strings and pointers for histogram loop "<<hi+1<<"... ";
      ...
      cout<<"histogram loop "<<hi+1<<" histogram created"<<endl;
      //draw histograms
      cout<<"drawing histogram "<<hi+1<<"... ";
      h[ci][hi]->SetLineColor(hi+1);
      xaxislabel=thisbranch->xlabel;
      yaxislabel=thisbranch->ylabel;
      ...
      //fill legends
      if(leglabel!="") leg[ci]->AddEntry(h[ci][hi],leglabel,"l");
      cout<<"done"<<endl;
    }//end histogram loop
    ...
    hs[ci]->Draw("nostack");
    hs[ci]->GetXaxis()->SetTitle(stacktitle);
    hs[ci]->GetYaxis()->SetTitle(yaxislabel);
    c[ci]->Update();
    if(leg[ci]->GetNRows()>0) leg[ci]->Draw();//This used to be if(nComparisons>0); 
    //                                          you don't need a legend if there's only one thing drawn
    cf->cd(ci+1);
    c[ci]->DrawClonePad();
    cout<<"done"<<endl;
    //save stuff:
    cout<<"saving files... ";
    placeholder = outputlocation+"f"+cistring+filename;
