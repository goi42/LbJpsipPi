...
// {"/afs/cern.ch/work/m/mwilkins/Lb2JpsiLtr/MC/withKScut/SMC_tuples_with_gd_info.root","SMCfile",f3quality}, \
  ...
  for(int ifile=0;ifile<nFiles;ifile++){
    ...
    for(int ibranch=0; ibranch<nBranches; ibranch++){
      ...
      //assign cuts
      TCut cLL,cDD,ctrigger,cnewest_noLM;
      makecuts(ifile,cLL,cDD,ctrigger,cnewest_noLM);

      thisbranch->c = {{cnewest_noLM,"newest_noLM"}};
	...
	if(thiscut->name.Contains(" ")){
	  ...
	  exit(EXIT_FAILURE);
        ...
        placeholder = fileoutputlocation+"cutfile_"+thiscut->name+"_LWM.root";
