...
void fit_mass() {
  TString fileN = "DD1";//suffix added before file extension, e.g., '.pdf'
  ...
  TH1F * h100 = (TH1F*)inHistos->Get("h80");
  ...
  TH1F *SMCh = (TH1F*)SMChistos->Get("h20");
  ...
  // Construct Pdf Model
  // /\0
  RooRealVar mean1L("mean1L","/\\ gaus 1: mean",5621.431,5525,5700);
  RooRealVar sig1L("sig1L","/\\ gaus 1: sigma",8.985,0,100);
  RooFormulaVar mean2L("mean2L","@0",RooArgList(mean1L));
  // RooRealVar mean2L("mean2L","/\\ gaus 2: mean",5621.302,5525,5700);
  RooRealVar sig2L("sig2L","/\\ gaus 2: sigma",28.680,0,100);
  RooGaussian gau1L("gau1L","/\\ signal: gaussian 1",*mass,mean1L,sig1L);
  RooGaussian gau2L("gau2L","/\\ signal: gaussian 2",*mass,mean2L,sig2L);
  RooRealVar f1L("f1L","/\\ signal: fraction gaus 1",0.749,0,1);
  ...
  // /\*
  cout<<"Lst stuff"<<endl;
  RooRealVar meanLst1("meanLst1","/\\*: mean1",5009.992,4500,5250);
  RooRealVar sigLst1("sigLst1","/\\*: sigma1",90.591,0,1000);
  RooRealVar meanLst2("mean5Lst2","/\\*: mean2",5255.467,5000,5350);
  RooRealVar sigLst2("sigLst2","/\\*: sigma2",60.855,0,1000);
  RooRealVar alphaLst2("alphaLst2","/\\*: alpha2",25.058,0,1000);
  RooRealVar nLst2("nLst2","/\\*: n2",10.201,0,1000);
  RooGaussian gauLst1("gauLst1","#Lambda* bkg: gaussian: misc.",*mass,meanLst1,sigLst1);
  RooCBShape CBLst2("CBLst2","#Lambda* bkg: CB: 1405",*mass,meanLst2,sigLst2,alphaLst2,nLst2);
  RooRealVar fLst1("fLst1","/\\* bkg: fraction gaus 1",0.743,0,1);
  RooAddPdf bkgLst("bkgLst","#Lambda* signal",RooArgList(gauLst1,CBLst2),RooArgList(fLst1));
  
  //Poly func BKG mass
  // RooRealVar b0("b0","Background: Chebychev b0",-1.071,-10000,10000);
  RooRealVar b1("b1","Background: Chebychev b1",-1.088,-10000,10000);
  RooRealVar b2("b2","Background: Chebychev b2",0.136,-10000,10000);
  RooRealVar b3("b3","Background: Chebychev b3",-0.102,-10000,10000);
  RooRealVar b4("b4","Background: Chebychev b4",0.030,-10000,10000);
  RooRealVar b5("b5","Background: Chebychev b5",-0.155,-10000,10000);
  RooRealVar b6("b6","Background: Chebychev b6",0.107,-10000,10000);
  // RooRealVar b7("b7","Background: Chebychev b7",0.018,-10000,10000);
  RooArgList bList(b1,b2);
  RooChebychev bkg("bkg","Background", *mass, bList);
  
  //number of each shape  
  RooRealVar nbkg("nbkg","N_{bkg}",5530.974,0,100000000);
  RooRealVar nsigL("nsigL","Yield #Lambda",4637.149,0,100000000);
