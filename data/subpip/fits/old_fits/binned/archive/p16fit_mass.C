...
void fit_mass() {
  TString fileN = "16";//suffix added before file extension, e.g., '.pdf'
  ...
  RooRealVar *mass = new RooRealVar("Bs_LOKI_MASS_JpsiConstr","m(J/#psi #Lambda)",4300,5950,"MeV");//adding range to variable 
  ...
  // /\0
  //gaussian
  RooRealVar mean1L("mean1L","/\\ gaus 1: mean",5621.319107,5525,5700);
  RooRealVar sig1L("sig1L","/\\ gaus 1: sigma",8.240592,0,100);
  RooGaussian gau1L("gau1L","#Lambda signal: gaussian 1",*mass,mean1L,sig1L);
  RooFormulaVar mean2L("mean2L","@0",mean1L);
  RooRealVar sig2L("sig2L","/\\ gaus 2: sigma",23.170207,0,100);
  RooGaussian gau2L("gau2L","#Lambda signal: gaussian 2",*mass,mean2L,sig2L);
  RooRealVar f1L("f1L","/\\ signal: fraction gaussian 1",0.832908,0,1);
  ...
  // /\*
  cout<<"Lst stuff"<<endl;
  RooRealVar meanLst1("meanLst1","/\\*(misc.): mean1",5009.190559,4900,5100);
  RooRealVar sigLst1("sigLst1","/\\*(misc.): sigma1",73.446488,0,100);
  RooRealVar meanLst2("mean5Lst2","/\\*(1405): mean2",5242.491202,5100,5350);
  RooRealVar sigLst2("sigLst2","/\\*(1405): sigma2",63.906158,0,100);
  RooRealVar alphaLst2("alphaLst2","/\\*(1405): alpha2",18.526057,0,50);
  RooRealVar nLst2("nLst2","/\\*(1405): n2",10.201000,0,50);
  RooGaussian gauLst1("gauLst1","#Lambda* bkg: gaus: misc.",*mass,meanLst1,sigLst1);
  RooCBShape gauLst2("gauLst2","#Lambda* bkg: CB: 1405",*mass,meanLst2,sigLst2,alphaLst2,nLst2);
  ...
  RooRealVar b1("b1","Background: Chebychev b1",-0.988736,-10,-0);
  RooRealVar b2("b2","Background: Chebychev b2",0.126528,0,10);
  ...
  RooRealVar nbkg("nbkg","N bkg",4157.547510,0,100000000);
  RooRealVar nsigL("nsigL","N /\\",4698.635911,0,1000000000);
  RooRealVar nsigS("nsigS","N sigma",69.793576,0,10000000000);
  RooRealVar ngauLst1("ngauLst1","N /\\*(misc.)",1665.794,0,10000000000);
  RooRealVar ngauLst2("ngauLst2","N /\\*(1405)",1667.099485,0,10000000000);
  ...
  int i=0;//color index
  TLegend *leg = new TLegend(0.2, 0.52, .4, .92);  
  ...
  while(vartPC){//loop over compotents of totalPdf
    ...
    if(!(vartPCtitle.Contains("aoeu:")||vartPCtitle.Contains("@"))){//only for non-sub-shapes
