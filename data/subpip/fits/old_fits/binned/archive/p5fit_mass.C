...
void fit_mass() {
  TString fileN = "5";//suffix added before file extension, e.g., '.pdf'
  ...
  // Construct Pdf Model
  // /\0
  RooRealVar mean1L("mean1L","/\\0 gaus 1: mean",5621.302524,5525,5700);
  RooRealVar sig1L("sig1L","/\\0 gaus 1: sigma",18.289123,0,100);
  RooFormulaVar mean2L("mean2L","@0",RooArgList(mean1L));
  RooRealVar sig2L("sig2L","/\\0 gaus 2: sigma",7.788756,0,100);
  ...
  RooRealVar f1L("f1L","/\\0 signal: fraction gaus 1",0.231366,0,1);
  ...
  // /\*
  cout<<"Lst stuff"<<endl;
  RooRealVar meanLst("mean5Lst","/\\*: mean",5030.324299,4800,5150);
  RooRealVar sigLst("sig5Lst","/\\*: sigma",51.821141,0,100);
  RooGaussian bkgLst("bkgLst","#Lambda* bkg",*mass,meanLst,sigLst);
  
  //Poly func BKG mass
  RooRealVar b1("b1","Background: Chebychev b1",-0.950094,-10000,-0.00000001);
  RooRealVar b2("b2","Background: Chebychev b2",-68.275185,-10000,-0.00000001);
  RooRealVar b3("b3","Background: Chebychev b3",-39.678173,-10000,-0.00000001);
  RooRealVar b4("b4","Background: Chebychev b4",-39.678173,-10000,-0.00000001);
  RooArgList bList(b1);
