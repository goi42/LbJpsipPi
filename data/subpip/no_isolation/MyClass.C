...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(((H1_TRACK_Type==3)&&(H2_TRACK_Type==3)&&(H1_TRACK_GhostProb<0.2)&&(H2_TRACK_GhostProb<0.2)\
        ...
        if((dwmassR<-40)||(dwmassR>40)){
          cutmassLb7->Fill(Bs_LOKI_MASS_JpsiConstr);
          //trigger cut
          if(((J_psi_1SHlt1DiMuonHighMassDecision_TOS==1)||(J_psi_1SHlt1TrackMuonDecision_TOS==1)|| \
              (J_psi_1SHlt1TrackAllL0Decision_TOS==1))&&(J_psi_1SHlt2DiMuonDetachedJPsiDecision_TOS==1)){
            cutmassLb13->Fill(Bs_LOKI_MASS_JpsiConstr);
          }
          if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770)){
            cutmassLb7yield++;
            pTLb->Fill(Bs_PT);
            float fLfd_ = 0.151+exp(-0.573-0.095 * Bs_PT/1000);
            fLfd->Fill(fLfd_);
            fLfdtot+=fLfd_;
          }
          if((H1_TRACK_Type==3)&&(H2_TRACK_Type==3)){
            cutmassLb8->Fill(Bs_LOKI_MASS_JpsiConstr);
            //trigger cut
            if(((J_psi_1SHlt1DiMuonHighMassDecision_TOS==1)||(J_psi_1SHlt1TrackMuonDecision_TOS==1)|| \
                (J_psi_1SHlt1TrackAllL0Decision_TOS==1))&&(J_psi_1SHlt2DiMuonDetachedJPsiDecision_TOS==1)){
              cutmassLb14->Fill(Bs_LOKI_MASS_JpsiConstr);
            }
            if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770)){
              cutmassLb8yield++;
              pTLbLL->Fill(Bs_PT);
              float fLfdLL_ = 0.151+exp(-0.573-0.095 * Bs_PT/1000);
              fLfdLL->Fill(fLfdLL_);
              fLfdLLtot+=fLfdLL_;
            }
          }
          if((H1_TRACK_Type==5)&&(H2_TRACK_Type==5)){
            cutmassLb9->Fill(Bs_LOKI_MASS_JpsiConstr);
            //trigger cut
            if(((J_psi_1SHlt1DiMuonHighMassDecision_TOS==1)||(J_psi_1SHlt1TrackMuonDecision_TOS==1)|| \
                (J_psi_1SHlt1TrackAllL0Decision_TOS==1))&&(J_psi_1SHlt2DiMuonDetachedJPsiDecision_TOS==1)){
              cutmassLb15->Fill(Bs_LOKI_MASS_JpsiConstr);
            }
            if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770)){
              cutmassLb9yield++;
              pTLbDD->Fill(Bs_PT);
              float fLfdDD_ = 0.151+exp(-0.573-0.095 * Bs_PT/1000);
              fLfdDD->Fill(fLfdDD_);
              fLfdDDtot+=fLfdDD_;
            }
          }
        ...
        if((H1_PIDp>10)&&((H1_PIDp-H1_PIDK)>3)){
          ...
          //WM cut:
          if((dwmassR<-40)||(dwmassR>40)){
            cutmassLb10->Fill(Bs_LOKI_MASS_JpsiConstr);
            if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770))cutmassLb10yield++;
            if((H1_TRACK_Type==3)&&(H2_TRACK_Type==3)){
              cutmassLb11->Fill(Bs_LOKI_MASS_JpsiConstr);
              if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770))cutmassLb11yield++;
            }
            if((H1_TRACK_Type==5)&&(H2_TRACK_Type==5)){
              cutmassLb12->Fill(Bs_LOKI_MASS_JpsiConstr);
              if((Bs_LOKI_MASS_JpsiConstr>=5470)&&(Bs_LOKI_MASS_JpsiConstr<=5770))cutmassLb12yield++;
            }
            //cutghost histogram:
   ...
   chi2ndf->Draw();
   canvas1->Print("output/plots/plots.eps(");
   TCanvas *canvas2 = new TCanvas("canvas2","GhostProb",1200,800);
   canvas2->Divide(2,2);
   cout<<"canvas 2/26 made"<<endl;
   canvas2->cd(1);
   gStyle->SetOptStat(kFALSE);
   ...
   canvas2->Print("output/plots/plots.eps(");
   TCanvas *canvas3 = new TCanvas("canvas3","cutGhostProb",1200,800);
   canvas3->Divide(2,2);
   cout<<"canvas 3/26 made"<<endl;
   canvas3->cd(1);
   gStyle->SetOptStat(kFALSE);
   ...
   cutmassLb4->Draw();
   canvas8->Print("output/plots/plots.eps(");
   TCanvas *canvas9 = new TCanvas("canvas9","cutmassLb5",1200,800);
   cout<<"canvas 9/26 made"<<endl;
   gStyle->SetOptStat("e");
   ...
   cutmassLb5->Draw();
   canvas9->Print("output/plots/plots.eps(");
   TCanvas *canvas10 = new TCanvas("canvas10","cutmassLb6",1200,800);
   cout<<"canvas 10/26 made"<<endl;
   gStyle->SetOptStat("e");
   ...
   cutmassLb6->Draw();
   canvas10->Print("output/plots/plots.eps(");
   TCanvas *canvas11 = new TCanvas("canvas11","cutmassLb7",1200,800);
   cout<<"canvas 11/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb7->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut (MeV)");
   cutmassLb7->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb7->Draw();
   cutmassLb7->Write();
   canvas11->Print("output/plots/plots.eps(");
   TCanvas *canvas12 = new TCanvas("canvas12","cutmassLb8",1200,800);
   cout<<"canvas 12/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb8->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut LL (MeV)");
   cutmassLb8->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb8->Draw();
   cutmassLb8->Write();
   canvas12->Print("output/plots/plots.eps(");
   TCanvas *canvas13 = new TCanvas("canvas13","cutmassLb9",1200,800);
   cout<<"canvas 13/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb9->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut DD (MeV)");
   cutmassLb9->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb9->Draw();
   cutmassLb9->Write();
   canvas13->Print("output/plots/plots.eps(");
   TCanvas *canvas14 = new TCanvas("canvas14","cutmassLb10",1200,800);
   cout<<"canvas 14/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb10->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts, pID cut, and KS cut (MeV)");
   cutmassLb10->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb10->Draw();
   canvas14->Print("output/plots/plots.eps(");
   TCanvas *canvas15 = new TCanvas("canvas15","cutmassLb11",1200,800);
   cout<<"canvas 15/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb11->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts, pID cut, and KS cut LL (MeV)");
   cutmassLb11->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb11->Draw();
   canvas15->Print("output/plots/plots.eps(");
   TCanvas *canvas16 = new TCanvas("canvas16","cutmassLb12",1200,800);
   cout<<"canvas 16/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb12->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts, pID cut, and KS cut DD (MeV)");
   cutmassLb12->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb12->Draw();
   canvas16->Print("output/plots/plots.eps(");
   TCanvas *canvas17 = new TCanvas("canvas17","masspipi",1200,800);
   cout<<"canvas 17/26 made"<<endl;
   gStyle->SetOptStat("e");
   masspipi->GetXaxis()->SetTitle("R_WM (MeV)");
   masspipi->GetYaxis()->SetTitle("Events / 5MeV");
   masspipi->Draw();
   canvas17->Print("output/plots/plots.eps(");
   TCanvas *canvas18 = new TCanvas("canvas18","pTLb",1200,800);
   cout<<"canvas 18/26 made"<<endl;
   gStyle->SetOptStat("e");
   pTLb->GetXaxis()->SetTitle("Bs_PT (MeV)");
   pTLb->GetYaxis()->SetTitle("Events / 5MeV");
   pTLb->Draw();
   canvas18->Print("output/plots/plots.eps(");
   TCanvas *canvas19 = new TCanvas("canvas19","pTLbLL",1200,800);
   cout<<"canvas 19/26 made"<<endl;
   gStyle->SetOptStat("e");
   pTLbLL->GetXaxis()->SetTitle("Bs_PT LL (MeV)");
   pTLbLL->GetYaxis()->SetTitle("Events / 5MeV");
   pTLbLL->Draw();
   canvas19->Print("output/plots/plots.eps(");
   TCanvas *canvas20 = new TCanvas("canvas20","pTLbDD",1200,800);
   cout<<"canvas 20/26 made"<<endl;
   gStyle->SetOptStat("e");
   pTLbDD->GetXaxis()->SetTitle("Bs_PT DD (MeV)");
   pTLbDD->GetYaxis()->SetTitle("Events / 5MeV");
   pTLbDD->Draw();
   canvas20->Print("output/plots/plots.eps(");
   TCanvas *canvas21 = new TCanvas("canvas21","fLfd",1200,800);
   cout<<"canvas 21/26 made"<<endl;
   gStyle->SetOptStat("e");
   fLfd->GetXaxis()->SetTitle("f_{#Lambda_{b}^{0}}/f_{d}(p_{T}) = 0.151+exp(-0.573-0.095 #times Bs_PT/1000)");
   fLfd->GetYaxis()->SetTitle("Events / 0.01");
   fLfd->Draw();
   canvas21->Print("output/plots/plots.eps(");
   TCanvas *canvas22 = new TCanvas("canvas22","fLfdLL",1200,800);
   cout<<"canvas 22/26 made"<<endl;
   gStyle->SetOptStat("e");
   fLfdLL->GetXaxis()->SetTitle("f_{#Lambda_{b}^{0}}/f_{d}(p_{T}) = 0.151+exp(-0.573-0.095 #times Bs_PT/1000)  LL");
   fLfdLL->GetYaxis()->SetTitle("Events / 0.01");
   fLfdLL->Draw();
   canvas22->Print("output/plots/plots.eps(");
   TCanvas *canvas23 = new TCanvas("canvas23","fLfdDD",1200,800);
   cout<<"canvas 23/26 made"<<endl;
   gStyle->SetOptStat("e");
   fLfdDD->GetXaxis()->SetTitle("f_{#Lambda_{b}^{0}}/f_{d}(p_{T}) = 0.151+exp(-0.573-0.095 #times Bs_PT/1000)  DD");
   fLfdDD->GetYaxis()->SetTitle("Events / 0.01");
   fLfdDD->Draw();
   canvas23->Print("output/plots/plots.eps)");
 TCanvas *canvas24 = new TCanvas("canvas24","cutmassLb13",1200,800);
   cout<<"canvas 24/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb13->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut and trigger cut (MeV)");
   cutmassLb13->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb13->Draw();
   cutmassLb13->Write();
   canvas24->Print("output/plots/yesplots.eps(");
   TCanvas *canvas25 = new TCanvas("canvas25","cutmassLb14",1200,800);
   cout<<"canvas 25/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb14->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut and trigger cut LL (MeV)");
   cutmassLb14->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb14->Draw();
   cutmassLb14->Write();
   canvas25->Print("output/plots/yesplots.eps(");
   TCanvas *canvas26 = new TCanvas("canvas26","cutmassLb15",1200,800);
   cout<<"canvas 26/26 made"<<endl;
   gStyle->SetOptStat("e");
   cutmassLb15->GetXaxis()->SetTitle("Bs_LOKI_MASS_JpsiConstr with Liming's cuts and KS cut and trigger cut DD (MeV)");
   cutmassLb15->GetYaxis()->SetTitle("Events / 5MeV");
   cutmassLb15->Draw();
   cutmassLb15->Write();
   canvas26->Print("output/plots/yesplots.eps)");

   gROOT->SetBatch(kFALSE);

   cout<<"creating yields.txt"<<endl;
   ofstream myfile;
   myfile.open("output/yields/yields.txt");
   cout<<"yields.txt created"<<endl;
   myfile<<"yields of 5470<=Bs_LOKI_MASS_JpsiConstr<=5770 with various cuts:"<<endl<<"no cuts: "<<massLbyield<<endl;
