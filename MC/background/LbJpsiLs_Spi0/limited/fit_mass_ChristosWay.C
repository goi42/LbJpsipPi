...
void fit_mass_ChristosWay() {
  ...
  TFile *inHistos;
  TH1F * h100;
  inHistos = new TFile("histos_180018101820.root", "READ");
  h100 = (TH1F*)inHistos->Get("massLb");

  double massLb = 5050;
  double massLbmin = 4700;
  double massLbmax = 5300;
  ...
  RooRealVar meanCB1("meanCB1","meanCB1",5120.32);
  RooRealVar sigCB1("sigCB1","sigCB1",95.8761);
  ...
  RooRealVar mean1("mean1","mean1",4911.26);
  RooRealVar sig1("sig1","sig1",105.228);
  RooRealVar mean2("mean2","mean2",4700);
  RooRealVar sig2("sig2","sig2",149.45);
  RooRealVar mean3("mean3","mean3",5272.59);
  RooRealVar sig3("sig3","sig3",65.3598);
  RooRealVar mean4("mean4","mean4",massLb,massLbmin,massLbmax);
  RooRealVar sig4("sig4","sig4",6,sigmin,sigmax);
  RooRealVar mean5("mean5","mean5",massLb,massLbmin,massLbmax);
  RooRealVar sig5("sig5","sig5",6,sigmin,sigmax);
  ...
  RooGaussian gau5("gau5","gau5",*mass,mean5,sig5);
  ...
  RooRealVar f4("f4","gau4_frac",0.1,0,1);
  RooAddPdf sig("sig","sig",RooArgList(gau1,gau2,gau3,gau4,gau5),RooArgList(f1,f2,f3,f4));
  ...
  totalPdf.plotOn(framex,Name("signal: gau5"),LineStyle(kDashed),LineColor(kCyan-1),Components(gau5));
  ...
  TLegend *leg = new TLegend(0.7, 0.5, .86, .9);
  ...
  leg->AddEntry(framex->findObject("signal: gau5"),"Signal: Gaussian5","l");
...
//list important parameters on the plot
  TPaveText* txt = new TPaveText(0.1,0.7,0.7,0.9,"BRNDC");
  ...
  txt->AddText("Yield #Lambda* = "+Y_nsigstring+" +/- "+E_nsigstring);
  ...
  std::cout << "mean5: " << mean5.getVal() <<endl;  
  ...
  std::cout << "sig5: " << sig5.getVal() <<endl;
  ...
  std::cout << "f4: " << f4.getVal() <<endl;
  //std::cout << "b1: " << b1.getVal() <<endl;
  // std::cout << "b2: " << b2.getVal() <<endl;
  
  can1->Print("plots/fit_5gau.pdf");
  can1->SaveAs("plots/fit_5gau.C");
